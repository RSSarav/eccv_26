...
# Usage:
#   ./utils/run_oft_finetune.sh <dataset_name> <lora_tag> [gpu_id]
# Example:
#   ./utils/run_oft_finetune.sh pick_up_red_cube_200 lora-r32 9
#
# Behavior:
# - Saves policies under: /bluesclues-data/home/pingpong-nima/raj/eccv_26/0_policies
# - Creates run folder: {dataset}_{lora}_{DD_MM_YY}
...
DATE="$(date +%d_%m_%y)"
TASK="pick_up_red_cube_200"
RUN_ID="${DATASET_NAME}_${LORA_TAG}_${DATE}"
...
echo "Starting OFT fine-tune run: ${RUN_ID}"
echo "Using GPU: ${GPU_ID}"
echo "Policy output folder: ${POLICY_DIR}"
echo "Log file: ${POLICY_DIR}/stdout.log"

CUDA_VISIBLE_DEVICES="${GPU_ID}" torchrun --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --wandb_project "${TASK}-openvla-oft-finetune" \
